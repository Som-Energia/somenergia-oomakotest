...
<p>&nbsp;</p>
<p>Por otro lado, debemos hacerte el <strong>retorno parcial</strong> de la aportaci&oacute;n que hiciste al Generation kWh. En este caso, es la liquidaci&oacute;n n&uacute;mero ${num_amortization} de 24, y es de ${format_currency(object.amount_total,'EUR', locale='es_ES')}:</p>
...
<p>Hola, ${object.partner_id.name.split(',')[-1]},</p>
<p>El projecte Generation kWh, en el qual participes des de ${date}, et permet accedir a la tarifa Generation kWh. Com que aquesta tarifa no dep&egrave;n del mercat, sin&oacute; de la gesti&oacute; i manteniment de les nostres plantes, l'any passat va ser considerablement m&eacute;s econ&ograve;mica que la tarifa est&agrave;ndard, que va patir la pujada dels preus de l'energia al mercat.</p>
<p>Aix&iacute; doncs, i tot i que <a href="https://ca.support.somenergia.coop/article/587-aportacio-generation-estalvi">no &eacute;s l&rsquo;objectiu</a> del projecte Generation kWh, gr&agrave;cies a la teva participaci&oacute;, el 2022 ${previous_year} et vas <strong>estalviar</strong> ${format_currency(irpf_values['irpf_saving'],'EUR', locale='es_ES')} euros.&nbsp;Aquest estalvi, que la llei considera guany en esp&egrave;cie, s&rsquo;ha de declarar a Hisenda i pagar la corresponent <strong>retenci&oacute;</strong> de l&rsquo;IRPF, en cas de persones f&iacute;siques, i de l&rsquo;impost de societats, en cas d&rsquo;empreses. En els dos casos aquesta retenci&oacute; &eacute;s del 19% sobre l&rsquo;estalvi obtingut. Des de Som Energia ja vam efectuar aquest pagament el mes de gener, per tant, en els pr&ograve;xims dies passarem a fer-te&rsquo;n la liquidaci&oacute; corresponent.</p>
...
<p>&nbsp;</p>
<p>D'altra banda, t'hem de fer el&nbsp;<strong>retorn parcial</strong> de l'aportaci&oacute; que vas fer al Generation kWh. En aquest cas, &eacute;s la liquidaci&oacute; n&uacute;mero ${num_amortization} de 24, i &eacute;s de ${format_currency(object.amount_total,'EUR', locale='ca_ES')}:</p>
...
<p dir="ltr">&nbsp;</p>
<p dir="ltr">En els propers dies procedirem a fer efectiu el pegament de l&rsquo;import al compte bancari seg&uuml;ent: ${bank_account}</p>
% else:
